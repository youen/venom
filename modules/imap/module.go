...
func (Module) GetDefaultAssertions() *venom.StepAssertions {
...
func (Module) Run(ctx venom.TestContext, step venom.TestStep) (venom.ModuleResult, error) {
	var e Module
	...
	result.Module.IMAPPassword = "****hidden****" // do not output password

	return venom.Dump(result)
}

func (e *Module) getMail() (*Mail, error) {
	...
	module.Debugf("count messages:%d", count)
	...
	for _, msg := range messages {
		m, erre := extract(msg)
		if erre != nil {
			module.Warnf("Cannot extract the content of the mail: %s", erre)
			continue
		}
		...
		if found {
			if e.DeleteOnSuccess {
				module.Debugf("Delete message %s", m.UID)
				if err := m.delete(c); err != nil {
					return nil, err
				}
			} else if e.MBoxOnSuccess != "" {
				module.Debugf("Move to %s", e.MBoxOnSuccess)
...
func (e *Module) isSearched(m *Mail) (bool, error) {
	if e.SearchFrom != "" {
...
func fetch(c *imap.Client, box string, nb uint32) ([]imap.Response, error) {
	module.Debugf("call Select")
	if _, err := c.Select(box, false); err != nil {
		module.Errorf("Error with select %s", err.Error())
	...
	if err != nil {
		module.Errorf("Error with fetch:%s", err)
		return []imap.Response{}, err
	}
	...
	module.Debugf("Nb messages fetch:%d", len(messages))
	return messages, nil
}
