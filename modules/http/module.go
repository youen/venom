...
func (Module) Run(ctx venom.TestContext, step venom.TestStep) (venom.ModuleResult, error) {
	...
	start := time.Now()
	module.Debugf("http.Run.doRequest> Begin")
	resp, err := client.Do(req)
	module.Debugf("http.Run.doRequest> End (%.3f seconds)", time.Since(start).Seconds())
	if err != nil {
...
func (e Module) getRequest(workdir string) (*http.Request, error) {
