...
func (Module) Run(ctx venom.TestContext, step venom.TestStep) (venom.ModuleResult, error) {
	...
	result.Module.Password = "****hidden****" // do not output password

	return venom.Dump(result)
}

func (e Module) produceMessages(workdir string) error {
	...
	module.Infof("Connecting Kafka cluster on adresses %v...", e.Addrs)
	...
	if e.MessagesFile != "" {
		module.Debugf("Preparing message from file %s", e.MessagesFile)
	...
	for i := range e.Messages {
		...
		module.Debugf("Preparing message '%s'", message)
...
func (e Module) consumeMessages() ([]Message, []interface{}, error) {
	if len(e.Topics) == 0 {
