...
func (Module) Run(ctx venom.TestContext, step venom.TestStep) (venom.ModuleResult, error) {
	...
	// Put script in file
	module.Debugf("work with tmp file %s", tmpscript.Name())
	...
	module.Debugf("teststep exec '%s %s'", shell, strings.Join(opts, " "))
	module.Infof("working directory: %s", wd)
	cmd.Dir = wd
