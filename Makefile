...
MODULES_BINARIES 					= 	$(addprefix $(TARGET_DIR)/$(EXEC)/, $(addsuffix _$(OS)_$(ARCH)$(if $(IS_WINDOWS),.exe), $(patsubst modules/%, %, $(EXEC))))	
CROSS_COMPILED_MODULES_BINARIES 	=	$(foreach OS, $(TARGET_OS), $(foreach ARCH, $(TARGET_ARCH), $(foreach EXEC, $(MODULES), $(MODULES_BINARIES))))
COMMON_FILES 						=	$(shell ls *.go lib/cmd/*.go)
EXECUTOR_COMMON_FILES				=  	$(shell find lib/module -type f -name "*.go" -print)
...
define get_module_path_from_binary_file
...
build: $(TARGET_DIR) $(CROSS_COMPILED_MODULES_BINARIES) $(CROSS_COMPILED_CORE_BINARIES)
	$(info *******************************************************)
...
$(CROSS_COMPILED_MODULES_BINARIES): $(COMMON_FILES) $(EXECUTOR_COMMON_FILES) $(call get_recursive_files, $@)
	$(info *** building module binary $@ from package $(call get_module_path_from_binary_file,$@))
	@$(MAKE) --no-print-directory  gobuild GOOS=$(call get_os_from_binary_file,$@) GOARCH=$(call get_arch_from_binary_file,$@) OUTPUT=$@ PACKAGE=$(call get_module_path_from_binary_file,$@)
...
test: $(TARGET_DIR) $(CROSS_COMPILED_MODULES_BINARIES) $(CROSS_COMPILED_CORE_BINARIES)
	go test -v ./... -coverprofile coverage.out -coverpkg ./...
	go tool cover -html coverage.out -o coverage.html

integration: $(TARGET_DIR) $(CROSS_COMPILED_MODULES_BINARIES) $(CROSS_COMPILED_CORE_BINARIES)
	$(TEST_KILL_SMTP)
	$(TEST_START_SMTP)
	$(TEST_KILL_KAFKA)
	$(TEST_START_KAFKA)
	$(TARGET_DIR)/venom_$(shell go env GOOS)_$(shell go env GOARCH) run --configuration-dir $(TARGET_DIR)/modules tests/*.yml --log debug
...
install: $(INSTALL_DIR)/venom $(VENOM_DIR) $(TARGET_DIR) $(CROSS_COMPILED_MODULES_BINARIES) $(CROSS_COMPILED_CORE_BINARIES)
	...
	$(info *** moving venom module binaries to $(VENOM_DIR) ...)
	@mv $(TARGET_DIR)/modules/* $(VENOM_DIR)
	@chmod -R +x $(VENOM_DIR)/*
	@rm -rf $(TARGET_DIR)
